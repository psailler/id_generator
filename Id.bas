Attribute VB_Name = "IdCdiscount"
Sub generateIdCdiscount()
...
For Each celV In Selection
    If celV.Value = Empty Then
        MsgBox "Il manque un ou plusieurs identifiants n'est renseigné pour générer le cashback", vbCritical + vbOKOnly, "Erreur"
        Exit Sub
    End If
Next
...
For Each mntEmpty In Selection
    If mntEmpty.Value = Empty Then
        MsgBox "Il manque un ou plusieurs montants de bons d'achats pour générer le cashback", vbCritical + vbOKOnly, "Erreur"
        Exit Sub
    End If
Next

For n = 2 To Sheets("CashbackGenerator").Range("C1048576").End(xlUp).Row
Sheets("Feuil1").Range("A" & n) = Sheets("CashbackGenerator").Range("C" & n) & ";" & Sheets("CashbackGenerator").Range("B" & n) * 100 & ";" & DateSerial(Year(Date), Month(Date) + 4, 1) - 1 & " 00:00:00"
Next n

Sheets("Feuil1").Select
Range("A1:A" & [A1048576].End(xlUp).Row).Select
Set cel = Columns(1).Find(what:="Introuvable", LookIn:=xlValues, lookat:=xlWhole)
